...
Разработал роут который удаляет все задачи пользователя по его id при помощи методологии TDD - разработка через тестирование.
Сначала написал тест `tdd_deleteUserTasks.test.js`, потом роут в `taskRoutes.js`, добавил функцию `deleteAllUserTasks` в `taskService.js` которая принимает `userId` и удаляет все задачи у этого пользователя.
Тест отдельно запускаю скриптом `"test-deleteUserTasks": "jest /tests/tdd_deleteUserTasks.test.js"` через `npm run test-deleteUserTasks`

---

добавил валидацию к роутам использовав express-validator

---

-   Создал фикстуры /tests/fixtures/fixtures.js - три тестовых объекта для юзера, таски и плана, и 4 функции-генератора - рандомных объектов юзеров, планов, тасок и сабтасок. Проверил работу фикстуры-генератора generateRandomUser в тестах - всё ок.
-   console.log() был замокан, проведён тест (самый первый)
